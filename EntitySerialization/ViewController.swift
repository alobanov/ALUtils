...
class DateToStringTransformer: ValueTransformer {
  ...
  override func transformedValue(_ value: Any?) -> Any? {
    ...
    let formatter = DateToStringTransformer.defaultDateFormatter
...
//    }
  ...
  func tetst() {
    ...
    let exporter = EntityExporter()
    var pppp = [String: Any]()
    if let ss = s {
      for p in ss {
        pppp = exporter.toJSON(p)
      }
    }
...
//    }
  ...
  func log() {
    ...
    if let ss = s {
      for p in ss {
        logger.debug(exporter.toJSON(p))
      }
    }
  }
  ...
  func objects<T: NSManagedObject>(type: T.Type, predicate: NSPredicate, sort: [NSSortDescriptor]?) -> [T]? {
    do {
      ...
      return try dataStack.mainContext.fetch(fetchRequest)
    } catch {
      return nil
    }
  }
